...
function runDailyDigest() {
  try {
    ...
    const subject = "miniFeedly Daily Digest - " + Utilities.formatDate(new Date(), "UTC", "yyyy-MM-dd");
...
function getSheetId_() {
  ...
  const ss = SpreadsheetApp.create("miniFeedly Data");
  ...
  Logger.log("miniFeedly Sheet created: " + ss.getUrl());
  return ss.getId();
}
